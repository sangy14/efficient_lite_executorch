...
BASE_OUTPUT_DIR = "efficientdet_lite0_vulkan_models"
...
# =============================================================================
def export_and_save_model(model, example_input, output_name, description, batch_size, use_vulkan=True):
    print("\n" + "=" * 70)
    ...
    try:
        with torch.no_grad():
            if isinstance(model, torch.export.ExportedProgram):
                ...
                print(" Model already exported program")
            else:
    ...
    try:
        if use_vulkan:
            ...
            print(" Vulkan partitioning succeeded")
        else:
...
def export_all_models():
    print("\n" + "=" * 70)
    print("EFFICIENTDET-LITE0 MODEL EXPORT (VULKAN BACKEND)")
    print("=" * 70)
    ...
    # We use a dummy input for 320x320 FP32 export
    ...
    try:
        ...
        with patch_batch_norm_for_fp32():
            _, fp32_file = export_and_save_model(
                ...
                "efficientdet_lite0_vulkan_fp32_320.pte",
                "FP32 MODEL (VULKAN)",
    ...
    try:
        ...
        with patch_batch_norm_for_fp32():
            _, q_file = export_and_save_model(
                ...
                "efficientdet_lite0_vulkan_int8_static_perchannel_320.pte",
...
if __name__ == "__main__":
    ...
    os.makedirs("efficientdet_results_vulkan", exist_ok=True)
    with open("efficientdet_results_vulkan/export_summary.json", "w") as f:
        json_results = {
            k: {
                "filename": v["filename"],
        ...
        print(f"\nExport summary saved to efficientdet_results_vulkan/export_summary.json")
