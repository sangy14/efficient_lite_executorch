...
BASE_OUTPUT_DIR = "efficientdet_lite4_exported_models"
...
def export_and_save_model(model, example_input, output_name, description,batch_size, use_xnnpack=True):
    ...
    def dealias_nodes(program):
        """Force specific output shapes for nodes known to cause XNNPACK mismatches."""
        SHAPE_MAP = {80: 56, 40: 28, 20: 14, 10: 7}
        
        from executorch.exir import EdgeProgramManager
        
        # Extract the graph based on the program type
        if isinstance(program, EdgeProgramManager):
            ep = program.exported_program("forward")
        else:
            ep = program

        count = 0
        for node in ep.graph.nodes:
            # Check both 'val' (used in Edge IR) and 'tensor_meta' (legacy)
            meta = node.meta.get('val') or node.meta.get('tensor_meta')
            if meta is not None and hasattr(meta, 'shape'):
                ...
                if len(shape) == 4:
                    ...
                    if h in SHAPE_MAP:
                        new_h = SHAPE_MAP[h]
                        new_shape = torch.Size([shape[0], shape[1], new_h, w])
                        # Update all possible metadata fields
                        if 'val' in node.meta:
                            try:
                                node.meta['val'] = node.meta['val']._replace(shape=new_shape)
                                count += 1
                            except:
                                pass
        ...
        if count > 0:
            print(f" -> De-aliased {count} nodes in the graph (forced spatial height shift).")
        return program
