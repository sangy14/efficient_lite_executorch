...
def transform_coordinates(x1, y1, x2, y2, orig_w, orig_h, input_size=640):
    """
    Transform coordinates from padded input space to original image space.
    Matches the preprocessing in inference_utils.preprocess_image.
    """
    scale = input_size / max(orig_w, orig_h)
    new_w = int(orig_w * scale)
    new_h = int(orig_h * scale)
    # Using top-left padding (letterboxing)
    left_pad = 0
    top_pad = 0
    
    # Transform: subtract padding and divide by scale
    # Note: Use the consistent scale used during preprocessing
    x_scale = scale
    y_scale = scale
    
    x1_orig = (x1 - left_pad) / x_scale
    y1_orig = (y1 - top_pad) / y_scale
    x2_orig = (x2 - left_pad) / x_scale
    y2_orig = (y2 - top_pad) / y_scale
    
    # Clip and convert to [x, y, w, h] for COCO
    # We clip the corners first, then calculate width and height
    x1_clipped = max(0.0, min(x1_orig, float(orig_w)))
    y1_clipped = max(0.0, min(y1_orig, float(orig_h)))
    x2_clipped = max(0.0, min(x2_orig, float(orig_w)))
    y2_clipped = max(0.0, min(y2_orig, float(orig_h)))
    
    x = x1_clipped
    y = y1_clipped
    w = max(0.0, x2_clipped - x1_clipped)
    h = max(0.0, y2_clipped - y1_clipped)
    
    # Ensure they are standard python floats for JSON serialization
...
def calculate_map_iou_metrics(detections: List,
    ...
    if annotations_path and os.path.exists(annotations_path) and num_detections > 0:
        try:
            # Get the list of image IDs present in detections to evaluate on a subset
            image_ids = list(set([d['image_id'] for d in detections]))
            
            # Pass detections list directly instead of writing to a temp file
            coco_metrics = calculate_coco_metrics(detections, annotations_path, img_ids=image_ids)
            if coco_metrics:
        ...
        logger.warning("COCO metrics failed or returned empty results. Check category IDs and image IDs.")
...
def calculate_iou(box1, box2, format='coco'):
...
def calculate_model_comparison_iou(pytorch_detections, et_detections):
    """
    Calculate average IoU between PyTorch and ExecuTorch detections for matching objects.
    """
    total_iou = 0.0
    num_matches = 0
    ...
    # Group by image_id
    py_by_img = {}
    for d in pytorch_detections:
        img_id = d['image_id']
        if img_id not in py_by_img: py_by_img[img_id] = []
        py_by_img[img_id].append(d)
        
    et_by_img = {}
    for d in et_detections:
        img_id = d['image_id']
        if img_id not in et_by_img: et_by_img[img_id] = []
        et_by_img[img_id].append(d)
        
    for img_id in py_by_img:
        if img_id not in et_by_img: continue
        
        py_dets = py_by_img[img_id]
        et_dets = et_by_img[img_id]
        
        # Simple matching: for each PyTorch box, find the closest ExecuTorch box of same class
        for py_d in py_dets:
            best_iou = 0.0
            for et_d in et_dets:
                if py_d['category_id'] == et_d['category_id']:
                    iou = calculate_iou(py_d['bbox'], et_d['bbox'], format='coco')
                    if iou > best_iou:
                        best_iou = iou
...
# ================================================================
def evaluate_pytorch_model(model, dataloader, device='cpu', max_samples=None, img_size=640):
    """
    ...
    Args:
        ...
        img_size: Input image size for coordinate transformation
    Returns:
            ...
            for b_idx, dets in enumerate(batch_detections):
                ...
                for det in dets:
                    ...
                    if x1 >= x2 or y1 >= y2:
                        continue
                    try:
                        # Diagnostic tests confirm the model outputs correct COCO IDs directly
                        category_id = int(class_id)
                        score_val = float(score)
                        bbox = transform_coordinates(x1, y1, x2, y2, orig_w, orig_h, input_size=img_size)
                        
                        if samples_processed < 2 and len(all_detections) < 3:
                            logger.info(f"DEBUG PyTorch: img={img_id}, cat={category_id}, score={score_val:.4f}, bbox={bbox}")
...
def evaluate_executorch_model(program, dataloader, max_samples=None, img_size=640):
    """
            ...
            with timer:
                ...
                for det in dets:
                    ...
                    try:
                        ...
                        if samples_processed < 2 and len(all_detections) < 3:
                            logger.info(f"DEBUG ExecuTorch: img={img_id}, cat={category_id}, score={score_val:.4f}, bbox={bbox}")
...
def main():
    ...
    if pte_files:
        ...
        for pte_file in pte_files:
            ...
            model_iou = calculate_model_comparison_iou(pytorch_results['detections'], et_results['detections'])
            et_results['metrics'] = et_metrics
            et_results['metrics']['model_comparison_iou'] = round(model_iou, 4)
            # Get model sizes
            ...
            logger.info(f"ExecuTorch mAP: {et_results['metrics'].get('mAP', 0):.4f}")
            logger.info(f"Model-to-Model IoU: {model_iou:.4f}")
